...
const WalletCashbackTable = () => {
  const [data, setData] = useState([]);
  const [filters, setFilters] = useState({ walletID: "", planID: ""});
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/admin/wallet-cashback`, {
        method: "POST",
        headers: {
      ...
      setData("");
      setError("Invalid Wallet ID or Plan ID. Please try again.");
    }
  };
...
export default WalletCashbackTable;
